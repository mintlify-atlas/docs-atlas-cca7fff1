...
description: "Guía de normalización de bases de datos relacionales: primera, segunda y tercera forma normal, BCNF y dependencias funcionales con ejemplos prácticos en SQL."
---
...
| `{cedula, codcurso} → nota_final` | La nota final depende de la combinación alumno-curso |
...
<Steps>
  ...
  <Step title="Aplicar la Segunda Forma Normal (2FN)">
    ...
    **Tabla que viola 2FN:**
    ...
    Supongamos que consolidamos la inscripción en una sola tabla con PK compuesta `{cedula, codcurso}`:

    | cedula | codcurso | nombre_alumno | nombrecurso | num_horas | nota_final |
    |--------|----------|---------------|-------------|-----------|------------|
    | 101 | C01 | Ana | SQL Avanzado | 40 | 4.5 |
    | 101 | C02 | Ana | Modelado ER | 30 | 3.8 |
    | 102 | C01 | Luis | SQL Avanzado | 40 | 4.0 |

    **Dependencias parciales:**
    - `cedula → nombre_alumno` (depende solo de cedula, no de la PK completa)
    - `codcurso → nombrecurso, num_horas` (depende solo de codcurso)
    - `{cedula, codcurso} → nota_final` (depende correctamente de toda la PK)
    ...
    **Solución — descomponer en tablas con dependencias completas:**
    ...
    -- La nota_final depende de la combinación alumno + curso
    CREATE TABLE alumno_curso (
      cedula      VARCHAR(15)   NOT NULL,
      codcurso    INTEGER       NOT NULL,
      nota_final  NUMERIC(3,1),
      CONSTRAINT pk_alumno_curso PRIMARY KEY (cedula, codcurso),
      CONSTRAINT fk_ac_alumno  FOREIGN KEY (cedula)
  ...
  <Step title="Aplicar la Tercera Forma Normal (3FN)">
    ...
    **Tabla que viola 3FN:**
    ...
    | codcurso | nombrecurso | cedprofesor | nombre_profesor | departamento_profesor |
    |----------|-------------|-------------|-----------------|----------------------|
    | C01 | SQL Avanzado | P01 | Martínez | Ingeniería |
    | C02 | Modelado ER | P02 | Rodríguez | Sistemas |
    | C03 | NoSQL | P01 | Martínez | Ingeniería |

    **Dependencia transitiva identificada:**
    ```
    codcurso → cedprofesor → nombre_profesor, departamento_profesor
    ```
